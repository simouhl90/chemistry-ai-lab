...
export function StarfieldCanvas() {
  ...
  useEffect(() => {
    ...
    const starColors = ['#ffffff', '#ffe4c4', '#c4d4ff', '#ffd700', '#00f0ff', '#ff9ff3'];
    ...
    const animate = () => {
      ...
      // Subtle nebula blobs
      const nebulaData = [
        { x: canvas.width * 0.2, y: canvas.height * 0.3, r: 250, color: '#1a0a2e' },
        { x: canvas.width * 0.7, y: canvas.height * 0.6, r: 300, color: '#0a1628' },
        { x: canvas.width * 0.5, y: canvas.height * 0.8, r: 200, color: '#0d1f2d' },
      ];
      ...
      for (let i = shootingStars.length - 1; i >= 0; i--) {
        ...
        if (ss.trail.length > 1) {
          ...
          ctx.globalAlpha = (1 - ss.life / ss.maxLife) * 0.6;
          ctx.lineWidth = 1.5;
          ctx.stroke();
        }

        // Head glow
        const headGrad = ctx.createRadialGradient(ss.x, ss.y, 0, ss.x, ss.y, 5);
        headGrad.addColorStop(0, '#ffffff');
        headGrad.addColorStop(1, 'transparent');
        ctx.fillStyle = headGrad;
        ctx.globalAlpha = (1 - ss.life / ss.maxLife) * 0.8;
        ctx.beginPath();
